...
from summ.models import Popular_crawled,Section_crawled,Comment_crawled,Popular_keyword,Section_keyword,Comment_keyword
...
def get_rankingNews(save_path, target_date, ranking_type) :
    ...
    if ranking_type == 'section' :
        ...
        date_list, press_list, ranking_list, num_list, title_list, link_list = get_rankingNews_infos(target_date=crawl_date, ranking_type=ranking_type)
    ...
    to_Database(ranking_type,data_list)
    clustering(save_path, data_list, ranking_type)


def get_rankingNews_infos(target_date, ranking_type, section=None, is_section=False) :
    ...
    press_id = {'경향신문' : '032', '국민일보' : '005', '동아일보' : '020', '서울신문' : '081', '조선일보' : '023', '중앙일보' : '025', '한겨레' : '028', '한국일보' : '469', 'JTBC' : '437', 'KBS' : '056', 'MBC' : '214', 'SBS' : '055', 'YTN' : '052', '연합뉴스' : '001', '매일경제' : '009', '서울경제' : '011', '아시아경제' : '277', '한국경제 ' : '015', '헤럴드경제' : '016'}
...
def clustering(save_path, data_list, ranking_type) :
    ...
    with open(f"{save_path}.json", 'w', encoding="utf-8") as f:
        f.write(j)
def to_Database(ranking_type,data_list):
    if ranking_type=='section':
        Section_database(data_list)
    else:
        Popular_database(data_list)

def Popular_database(data_list) :
    data = {}
    for idx in range (0, len(data_list[0])) :
        data = {
                'date' : data_list[0][idx],
                ...
                'ranking' : data_list[2][idx],
                'views' : data_list[3][idx],
                'section' : data_list[8][idx],
                'title' : data_list[4][idx],
                'link' : data_list[5][idx],
                'content' :data_list[6][idx],
                'keywords': ','.join(data_list[7][idx]),
            }
        Popular_crawled(data).save()
        key=Popular_crawled.object.latest('id')
        for i in range(0,len(data_list[7][idx])):
            keyword_data={
                'news_id':key,
                'keyword':data_list[7][idx][i],
            }
        Popular_keyword(keyword_data).save()       
    print("new item added!")

def Comment_database(data_list) :
    data = {}
    for idx in range (0, len(data_list[0])) :
        data = {
                "date" : data_list[0][idx],
        ...
        Comment_crawled(data).save()
        key=Comment_crawled.object.latest('id')
        for i in range(0,len(data_list[7][idx])):
            keyword_data={
                'news_id':key,
                'keyword':data_list[7][idx][i],
            }
            Comment_keyword(keyword_data).save()      
        print("new item added!")

def Section_database(data_list) :
    data = {}
    for idx in range (0, len(data_list[0])) :
        data = {
                'date' : data_list[0][idx],
                ...
                'link' : data_list[6][idx],
                'content' :data_list[7][idx],
                'keywords': ','.join(data_list[8][idx]),
            }
        Section_crawled(data).save()
        key=Section_crawled.object.latest('id')
        for i in range(0,len(data_list[8][idx])):
            keyword_data={
                'news_id':key,
                'keyword':data_list[8][idx][i],
            }
            Section_keyword(keyword_data).save()    
        print("new item added!")


def main():
    target_date = "20220330"
    save_path = "./crawling_result"
    file=save_path+"/언론사별 랭킹뉴스/"+target_date+"_언론사별 랭킹뉴스"
    get_rankingNews(save_path=f"{file}", target_date=target_date, ranking_type='popular')
    get_rankingNews(save_path=f"{file}", target_date=target_date, ranking_type='section')
    # get_rankingNews(save_path=f"{save_path}/언론사별 랭킹뉴스/{target_date}_언론사별 랭킹뉴스", target_date=target_date, ranking_type='comment')
